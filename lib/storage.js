...
Storage.prototype.uploader = function(req, res) {
	...
	var saveFileBlocks = function(uid, block_count, buf) {
		try {
			console.log('W', uid, block_count, 'BLOCKS', url.parse(req.headers['referer']).host);
		} catch (err) {
		}
...
Storage.prototype.readFile = function(req, res) {
	...
	var sendFile = function(info, buf) {
		...
		if (info.type && info.type.indexOf('image') === 0 && query && query.split('x').length === 2) {
			...
			console.log(1, info.link || info._id, buf);

			res.send(buf);
		}

		// CACHE
		cache.save(info.link || info._id, buf);
	};
	...
	var readFileBlocks = function(info) {
		self.db.collection('storage-blocks').get({id: info._id}, function(blocks) {
			...
			try {
				console.log('R', info._id, blocks.length, 'BLOCKS', url.parse(req.headers['referer']).host);
			} catch (err) {
			}
	...
	this.db.collection('storage-info').one({_id: fid}, function(info) {
		...
		cache.read(info.link || info._id, function(err, buf) {
			if (!err) {
				try {
					console.log('C', info._id, url.parse(req.headers['referer']).host);	
				} catch (e) {
					console.log('C', info._id);
				}	
			}

			console.log(2, info.link || info._id, buf);

			// if (!err)
			// 	return sendFile(info, buf);
